...
bool WriteSystemBitmap::carregarArquivo(std::string ARQUIVO) 
{
    FILE *ARQ_FONTE;
    std::string txt="";
...
void WriteSystemBitmap::setDimensao(int largura, int altura) 
{
    for (int i=0;i<256;i++){
...
int WriteSystemBitmap::getAltura() 
{
    return dimensaoPadrao.h;
}
//Retorna a dimensão (altura e largura) padrão
Dimensao WriteSystemBitmap::getDimensao() 
{
    return dimensaoPadrao;
}
//Desenha o texto na tela
void WriteSystemBitmap::escrever(const char * PALAVRA, int X, int Y) 
{
...
void WriteSystemBitmap::checkar() 
{
    int maior=0;
    for (int i=0; i<256; i++){
        if (largura[i]>maior){
